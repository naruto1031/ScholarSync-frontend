...
export default async function Entrance() {
	...
	const isAdministrator =
		session?.user.groups?.includes(teacherGroupID) && session?.user.groups?.includes(studentGroupID)
	return (
		<Container>
			<h1>Entrance</h1>
			...
			{(isTeacher || isAdministrator) && <Link href={'/entrance/teacher'}>教員ページへ</Link>}
		</Container>
	)
}
